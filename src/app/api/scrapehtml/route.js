...
export async function GET(request) {
  ...
  try {
    ...
    console.log(`[/api/scrapehtml] Incoming request for URL: ${url}, noCache: ${noCache}`);

    // Validate URL parameter
    if (!url) {
      console.error('[/api/scrapehtml] No URL provided');
    ...
    if (!validateUrl(url)) {
      console.error(`[/api/scrapehtml] Invalid URL: ${url}`);
      return NextResponse.json(
    ...
    try {
      ...
      console.error(`[/api/scrapehtml] URL sanitization failed: ${sanitizeError.message}`);
      return NextResponse.json(
    ...
    console.log(`[/api/scrapehtml] Scraping ${sanitizedUrl} for HTML content`);

    // Scrape the page and return HTML
    const html = await scrapePage(sanitizedUrl, 'html');

    const duration = Date.now() - startTime;
    console.log(`[/api/scrapehtml] Successfully scraped ${sanitizedUrl} in ${duration}ms`);
    ...
    const duration = Date.now() - startTime;
    console.error(`[/api/scrapehtml] Error after ${duration}ms:`, {
